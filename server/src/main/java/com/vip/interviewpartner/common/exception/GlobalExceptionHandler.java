...
import com.vip.interviewpartner.common.ApiResponse;
...
/**
 * 애플리케이션에서 발생하는 예외들을 전역적으로 처리하는 핸들러입니다.
 * 이 클래스는 REST 컨트롤러에서 발생할 수 있는 예외들을 적절히 포착하여 정해진 응답 형태로 변환합니다.
...
public class GlobalExceptionHandler {
     ...
     * 이 메서드는 시스템 내부에서 발생하는 예상치 못한 예외들을 처리하고, 내부 서버 오류를 클라이언트에 알립니다.
     *
     ...
     * CustomException에는 개발자가 정의한 에러 코드가 포함되어 있으며, 이를 바탕으로 상세한 오류 응답을 제공합니다.
     *
    ...
    /**
     * 입력 값 검증 실패(MethodArgumentNotValidException)를 처리합니다.
     * 이 메서드는 입력 값에 대한 검증 실패 시 발생하는 예외를 캡처하고, 상세한 검증 오류 정보를 클라이언트에 반환합니다.
     *
